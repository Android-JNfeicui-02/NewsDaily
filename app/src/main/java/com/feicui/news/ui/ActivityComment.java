...
public class ActivityComment extends MyBaseActivity {
		...
		public void onClick(View v) {
			...
			case R.id.imageview:// �������۰�ť
				...
				if (ccontent == null || ccontent.equals("")) {
					Toast.makeText(ActivityComment.this, "要先写评论内容哦，亲！",
				...
				if (TextUtils.isEmpty(token)) {
					Toast.makeText(ActivityComment.this, "对不起，您还没有登录.", 0)
							.show();
					return;
				}
	...
	protected void loadPreComment() {
		...
		if (SystemUtils.getInstance(this).isNetConn()) {
			...
			// entity修改，先注释
			// CommentsManager.loadComments(mode,comment.cid,comment.cnid,new
	...
	/*private class MyJsonHttpResponseHandler extends TextHttpResponseHandler {
		...
		public void onSuccess(int statusCode, Header[] headers,
			...
			if (statusCode == 200) {
				List<Comment> comments = ParserComments
						.parserComment(responseString);
				if (comments == null || comments.size() < 1) {
					return;
				}
				boolean flag = mode == NewsManager.MODE_NEXT ? true : false;
				 ...
				 * if(mode==NewsManager.MODE_NEXT){ boolean isClear=false;
				 * if(comments.size()>=12) isClear=true;
				 * adapter.appendDataTop(comments, true); }else
				 * if(mode==NewsManager.MODE_PREVIOUS){
